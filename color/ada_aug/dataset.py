...
def get_dataloaders(dataset, cfg):
    labels = labels = [dataset[i][1] for i in range(len(dataset))]
    sss = StratifiedShuffleSplit(n_splits=1, test_size=cfg.ada_aug.search_ratio, random_state=0)  # Adjust test_size as needed
    train_idx, search_idx = next(sss.split(list(range(len(dataset))), labels))
    train_set = Subset(dataset, train_idx)
    search_set = Subset(dataset, search_idx)
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=cfg.train.batch_size, shuffle=True)
    search_loader = torch.utils.data.DataLoader(search_set, batch_size=cfg.train.batch_size, shuffle=False)
    return train_loader, search_loader
...
#     return trainloader, validloader, searchloader
...
def get_label_name(dset, dataroot='data'):
    if 'cifar10' in dset:
...
class AdaAugDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, adaaug):
        self.adaaug = adaaug
        self.dataset = dataset
    ...
    def __getitem__(self, idx):
        ...
        data, target = data.to(self.adaaug.device), target.to(self.adaaug.device)
        data = self.adaaug(data, mode='exploit')
        
        return data, target

def augment_dataloader(dataloader, adaaug, cfg):
    num_workers = cfg.no_workers
    num_workers = num_workers * torch.cuda.device_count()
    
    dataset = dataloader.dataset
    augmented_dataset = AdaAugDataset(dataset, adaaug)
